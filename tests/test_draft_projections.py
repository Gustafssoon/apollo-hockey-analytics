...
def test_three_season_skater_projection_uses_availability_and_age_adjusted_rates(tmp_path):
    ...
    assert projection.model_version == "apollo-skater-baseline-v0.8"
    ...
    assert projection.pp_deployment_model_version is None
...
def test_draft_project_cli(tmp_path, capsys):
    ...
    assert "apollo-skater-baseline-v0.8" in output
    ...
    assert "PP deployment: not applied (context unavailable)" in output
