...
def run_age_baseline_backtest(
    ...
    *,
    ...
    evaluated: list[AgeBacktestPlayer] = []
    ...
    for player_id, seasons_by_stat in stats_by_player.items():
        ...
        age_history = tuple(
            (
                _age_on_date(birth_date, _season_start_date(season.season)),
                season.games_played,
                season.stats["goals"] + season.stats["assists"],
            )
    ...
    if base_eligible_players <= 0:
        raise ProjectionError("Age baseline shootout found no complete-history skaters")

    return build_age_backtest_result(
        target_season=target_season,
