...
def run_regression_backtest(
    ...
    *,
    ...
    for player_id, seasons_by_stat in stats_by_player.items():
        position = player_meta[player_id][3]
        for season in source_seasons:
            season_stats = seasons_by_stat.get(season, {})
            games_played = season_stats.get("gamesPlayed", 0.0)
            if games_played <= 0:
                continue
            prior_rows.append((season, position, games_played, season_stats))
    priors = build_position_priors(tuple(prior_rows))

    actual_required = ("gamesPlayed", *SKATER_PROJECTION_STATS)
    evaluated: list[RegressionBacktestPlayer] = []
    ...
    for player_id, seasons_by_stat in stats_by_player.items():
        ...
        regression_history: list[RegressionHistorySeason] = []
        ...
        evaluated.append(
            RegressionBacktestPlayer(
                ...
                history=tuple(regression_history),
