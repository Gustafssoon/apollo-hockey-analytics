...
from apollo.draft.goalie_rate_candidate import GOALIE_RATE_VARIANTS, apply_rate_regression
...
def _in_cohort(
    ...
    if cohort.min_age is not None and (age is None or age < cohort.min_age):
        return False
    return not (cohort.max_age is not None and (age is None or age >= cohort.max_age))


def run_goalie_rate_candidate_gate_season(
    database: Database,
    target_season: int,
) -> tuple[GoalieRateGateSeasonResult, ...]:
    database.initialize()
    ...
    candidate_specs = tuple(
        next(spec for spec in GOALIE_RATE_VARIANTS if spec.name == candidate_name)
    ...
    eligible = {cohort.name: 0 for cohort in GOALIE_RATE_GATE_COHORTS}
    baselines: dict[str, list[GoalieBacktestPlayer]] = {
        cohort.name: [] for cohort in GOALIE_RATE_GATE_COHORTS
    }
    candidates = {
        cohort.name: {candidate_name: [] for candidate_name in GOALIE_RATE_GATE_CANDIDATES}
    ...
    for player_id, seasons_by_stat in stats_by_player.items():
        ...
        candidate_players = {
            spec.name: apply_rate_regression(baseline, spec, priors[spec.stat_name])
            for spec in candidate_specs
        }
        for cohort in memberships:
            baselines[cohort.name].append(baseline)
            for spec in candidate_specs:
                candidates[cohort.name][spec.name].append(candidate_players[spec.name])
            ...
            candidates=tuple(
                GoalieRateGateSeasonCandidate(
                    ...
                    result=build_goalie_backtest_result(
                        target_season=target_season,
                        players=tuple(candidates[cohort.name][spec.name]),
                        actual_eligible_goalies=eligible[cohort.name],
                    ),
                )
                for spec in candidate_specs
...
def run_goalie_rate_candidate_gate(
    database: Database,
    ...
    *,
    ...
    season_results = tuple(
        ...
        for result in run_goalie_rate_candidate_gate_season(database, target_season)
    )
    return build_goalie_rate_gate_aggregate(season_results)
