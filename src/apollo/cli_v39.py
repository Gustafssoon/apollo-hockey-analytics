...
from apollo import cli_v38 as v38_cli
from apollo.db import Database
from apollo.draft.pp_deployment import PP_DEPLOYMENT_MODEL_VERSION
from apollo.draft.pp_deployment_candidate_gate import PP_DEPLOYMENT_CANDIDATE_VERSION
from apollo.draft.projections import MODEL_VERSION, ProjectionError
from apollo.services.pp_deployment_production_gate import run_pp_deployment_production_gate
...
def build_parser() -> argparse.ArgumentParser:
    ...
    gate_parser = draft_subparsers.add_parser(
        "pp-deployment-production-gate",
        help="Verify production v0.8 exactly reproduces the approved PP TOI/GP 5% candidate",
    )
...
def _draft_pp_deployment_production_gate(args: argparse.Namespace) -> None:
    result = run_pp_deployment_production_gate(
        Database(args.db),
        args.season,
        years=args.years,
    ...
    print("APOLLO PP TOI/GP 5% PRODUCTION v0.8 GATE")
    print()
    print(
        "Target seasons: "
    ...
    print(f"PP deployment layer: {PP_DEPLOYMENT_MODEL_VERSION}")
    print(f"Approved candidate: {PP_DEPLOYMENT_CANDIDATE_VERSION}")
    print("Missing context uses exact pre-PP-deployment numerical fallback.")
    print()
    ...
    print()
    print(
        f"{'N':>5} {'PPP MAE':>8} {'GAIN':>7} {'PPP+':>5} {'WORST':>8} "
        f"{'PPP RHO':>7} {'OTHER':>6}"
    )
    print(
        f"{cohort.player_seasons:>5} {ppp.candidate_mae:>8.3f} {ppp.mae_gain:>+7.3f} "
...
def main(argv: list[str] | None = None) -> None:
    ...
    if args.command != "draft" or args.draft_command != "pp-deployment-production-gate":
        v38_cli.main(argv)
        return
    try:
        _draft_pp_deployment_production_gate(args)
    except ProjectionError as error:
        raise SystemExit(f"PP deployment production gate error: {error}") from error
